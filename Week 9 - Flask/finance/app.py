...
def index():
    ...
    # r evise stock prices and portfilio values
    ...
    portfolio = db.execute(
        "SELECT * FROM portfolios JOIN stocks ON stocks.stock_id = portfolios.stock_id WHERE person_id=?", session["user_id"])
            ...
            if lookup(request.form.get("symbol")) != None:
                ...
                if value > cash:
                    ...
                    if not stock_id:  # if stock not yet in database
                        ...
                        db.execute("INSERT INTO stocks (symbol, companyname, price) VALUES (?, ?, ?)",
                                   symbol, result["name"], price)
                        ...
                        db.execute("INSERT INTO portfolios VALUES (?, ?, ?, ?)", session["user_id"],
                                   stock_id[0]["stock_id"], volume, total)
                    else:
                        ...
                        if not stock_in_port:
                            ...
                            db.execute("INSERT INTO portfolios VALUES (?, ?, ?, ?)",
                                       session["user_id"], stock_id[0]["stock_id"], volume, total)
                        else:
                    ...
                    db.execute("INSERT INTO transactions (person_id, symbol, transaction_price, volume, time) VALUES (?, ?, ?, ?, ?)",
                               session["user_id"], symbol, price, volume, time)
    ...
    if request.method == "GET":
        ...
        if not request.form.get("shares"):
            ...
            if type(volume) != int or volume < 0:
                return apology("invalid number of shares")
            symbol = request.form.get("symbol")  # I use stock_id but Check50 function requires a field with the name symbol
            stock_id = db.execute("SELECT * FROM stocks WHERE symbol=?", symbol)[0]["stock_id"]
